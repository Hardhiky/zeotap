...
function saveSpreadsheet() {
  localStorage.setItem('spreadsheet', JSON.stringify({
    data: spreadsheetData,
    formulas: formulaInCell,
    dependencies: executionsDependentOnCell,
...
function loadSpreadsheet() {
  ...
  if (saved) {
    ...
    formulaInCell = data.formulas;
    executionsDependentOnCell = data.dependencies;
    cellProperties = data.styles;
    DrawFunctions.drawGrid();
  }
}

function setTextAlign(align) {
  cellProperties[selectedCell.row][selectedCell.col].textAlign = align;
  DrawFunctions.drawGrid();
}

function setBold() {
  cellProperties[selectedCell.row][selectedCell.col].fontWeight = 
    cellProperties[selectedCell.row][selectedCell.col].fontWeight === 'bold' 
    ? 'normal' 
    : 'bold';
  DrawFunctions.drawGrid();
}
...
function setItalic() {
  cellProperties[selectedCell.row][selectedCell.col].fontStyle = 
    cellProperties[selectedCell.row][selectedCell.col].fontStyle === 'italic' 
    ? 'normal' 
    : 'italic';
  DrawFunctions.drawGrid();
}

// Update your formula input handling
document.getElementById('formula-input').addEventListener('input', (e) => {
  const formula = e.target.value;
  if (formula.startsWith('=')) {
    // Update formula handling logic
  }
});

// Add to your DrawFunctions
function drawGrid() {
  // ... existing code ...
  
  // Draw header row background
  ctx.fillStyle = '#f1f1f1';
  ctx.fillRect(0, 0, canvas.width, rowHeights[0]);
  
  // Draw column headers
  for (let col = 1; col < colCount; col++) {
    const cellName = convertRowColToCellName(0, col);
    const pos = getCellPosition(0, col);
    ctx.fillText(cellName, pos.x + 5, pos.y + rowHeights[0]/2);
  }
  
  // Draw row headers
  for (let row = 1; row < rowCount; row++) {
    const cellName = convertRowColToCellName(row, 0);
    const pos = getCellPosition(row, 0);
    ctx.fillText(cellName, pos.x + 5, pos.y + rowHeights[row]/2);
  }
}
